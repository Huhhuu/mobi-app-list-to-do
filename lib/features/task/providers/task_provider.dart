...
class TaskProvider extends ChangeNotifier {
  ...
  /// Task vừa bị xóa gần nhất, dùng cho Undo.
  ...
  Future<void> deleteTask(String taskId) async {
    try {
      // Lưu lại task trước khi xóa để hỗ trợ Undo
      final taskToDelete = _tasks.where((t) => t.id == taskId).firstOrNull;
  ...
  /// Hoàn tác xóa task vừa thực hiện.
  Future<void> undoDelete() async {
    final deleted = _lastDeletedTask;
    ...
    try {
      await _taskService.createTask(
        title: deleted.title,
        isMyDay: deleted.isMyDay,
        isImportant: deleted.isImportant,
        listId: deleted.listId,
        dueDate: deleted.dueDate?.toIso8601String(),
        reminderAt: deleted.reminderAt?.toIso8601String(),
      );
      await _reload();
    } catch (e) {
      _setError('Không thể hoàn tác: ${e.toString()}');
    }
  }
